...
@section('content')
    ...
    <section aria-labelledby="profil-siswa">
        <!-- Header -->
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
            <h1 id="profil-siswa" class="text-2xl lg:text-3xl font-bold text-gray-900">Profil Saya</h1>
        </div>
            ...
            <form method="POST" action="{{ route('siswa.profil.update') }}" enctype="multipart/form-data">
                    ...
                    <div class="flex flex-col sm:flex-row items-center gap-6">
                        ...
                        <div class="flex-1 text-center sm:text-left">
                            ...
                            <p class="text-sm text-gray-500 mb-2">
                                @if($user->kelas)
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-primary/10 text-primary">Kelas: {{ $user->kelas }}</span>
                                @else
                ...
                <div class="px-6 py-6 space-y-6">
                    ...
                    <div>
                        ...
                        <select name="kelas" id="kelas" class="w-full border rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-primary/30 @error('kelas') border-red-500 @enderror">
                            <option value="">Pilih Kelas</option>
                            @foreach($classes as $class)
                                <option value="{{ $class->name }}" {{ old('kelas', $user->kelas) == $class->name ? 'selected' : '' }}>
                                    {{ $class->name }}
                                    @if($class->level)
                                        ({{ $class->level }})
                                    @endif
                        ...
                        <p class="text-xs text-gray-500 mt-1">Pilih kelas Anda untuk melihat ujian yang sesuai dengan kelas Anda</p>
                        @error('kelas')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex flex-col sm:flex-row justify-end gap-3">
                    <a href="{{ route('siswa.dashboard') }}" class="px-4 py-2 rounded-lg border border-gray-300 bg-white text-gray-700 hover:bg-gray-50 transition-colors text-center">
                        Batal
                    </a>
