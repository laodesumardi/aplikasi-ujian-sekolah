...
<body>
    ...
    <script>
        ...
        window.addEventListener('beforeunload', function(e) {
            const answeredCount = Object.keys(answers).filter(qId => answers[qId] && answers[qId].trim() !== '').length;
            if (!isSubmittingExam && !hasFinishedExam && answeredCount < totalQuestions) {
                refreshAttempts++;
        ...
        function updateNavBox(i) {
            ...
            if (isActive) className += ' bg-primary text-white border-primary';
            ...
            else className += ' bg-white text-gray-700 border-gray-300';
        ...
        nextBtn.addEventListener('click', () => { if (currentIndex < totalQuestions - 1) { currentIndex++; renderQuestion(currentIndex); } });
        ...
        // Timer
        const startedAt = new Date('{{ $examResult->started_at ?? now() }}');
